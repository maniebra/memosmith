...
- **YouTube**: `Settings > Integrations > YouTube` toggles `features.youtube`. Pasting a lone YouTube link inserts `![YouTube](url)`, a normal media line (align/width via `|center|480`), which `mediaPreview` renders as a player (`src/lib/utils/youtube.ts`). The iframe loads a wrapper page from a loopback server (`src-tauri/src/youtube.rs`, port fetched in `main.ts` before mount): YouTube refuses to play without a `Referer` (error 153), and the `tauri://` origin sends none. On Linux the webview decodes through GStreamer; without `gst-libav` (H.264) YouTube cannot play, so `youtube_decodable` (asks `gst-inspect-1.0`) swaps the player for a card saying so. Offline (`navigator.onLine`), the player becomes a card linking out; the editor re-renders on `online`/`offline`.
- **Spotify**: `Settings > Integrations > Spotify` toggles `features.spotify`. Pasting a lone `open.spotify.com` link (or `spotify:` URI) inserts `![Spotify](url)`, rendered by `mediaPreview` as the `open.spotify.com/embed/...` iframe (`src/lib/utils/spotify.ts`); offline it becomes a card linking out.
- **Players survive typing**: the editor re-renders its HTML on every keystroke, and an iframe reloads whenever it leaves the document. `markdown-editor/players.ts` swaps each rendered player for a same-size placeholder and parks the live iframe (kept per `src`) in the overlay layer, positioned on the placeholder during the database layout pass.
- **Media menu**: right-clicking an image or player preview opens `mediaMenu.ts` (align, plus "Copy link" for YouTube/Spotify). Right clicks inside player iframes never reach the page; on Linux `src-tauri/src/frame_menu.rs` catches WebKit's `context-menu` signal and `src/lib/tauri/frameMenu.ts` replays it as a `contextmenu` event at that point.
- **Tests**: `gitlab.test.ts`, `github.test.ts`, `youtube.test.ts`, `spotify.test.ts`, `slashMenu.test.ts`.
