...
import { fenceContext, hasLanguageServer, rankCompletions, shouldComplete, wordPrefix } from "./lsp";

const note = "intro\n\n```python\nvalue = 1\nva\n```\n\ntail\n";
const caret = note.indexOf("va\n```") + 2;
const context = fenceContext(note, caret);

assert(context?.language === "python", "the fence language comes from its opening line");
assert(context?.code === "value = 1\nva", "the block source stops before the closing fence");
assert(context?.line === 1, "lines are counted from the first line inside the fence");
...
assert(fenceContext(note, note.length - 2) === null, "text after the fence has no context");
assert(
  fenceContext("```js\nlet a\n", 8)?.code === "let a\n",
...
assert(hasLanguageServer("haskell") === false, "unknown languages have no server");

assert(wordPrefix("value.toUp", 10) === "toUp", "the prefix stops at the member access");
...
assert(ranked[0].label === "append", "a prefix match sorts ahead of a middle match");
