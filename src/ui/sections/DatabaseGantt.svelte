<script lang="ts">
  ...
  $: span = (() => {
    ...
    return {
      ...
      days: Array.from(
        { length: days },
        (_, index) => new Date(first + index * dayMs),
      ),
    };
  })();
  ...
  /** Bars placed on the day grid: left edge and width, both in whole days. */
  $: placed = span
    ? bars.map((bar) => {
        const left = offset(bar.start, span.first);
        return {
          ...bar,
          style: `left: ${left * dayWidth}px; width: ${
            (offset(bar.end, span.first) - left + 1) * dayWidth
          }px`,
          label: `${rowTitle(bar.row, columns)} · ${isoDate(
            bar.start,
          )} → ${isoDate(bar.end)}`,
        };
      })
...
{:else}
  <div class="min-h-0 flex-1 overflow-auto p-3">
    <div class="flex w-max">
      ...
      <div>
        <div
          class="flex h-6 border-b border-stone-200/70 dark:border-stone-800"
        >
        ...
        {#each placed as bar (bar.row.id)}
          <div
            class="relative h-7"
            style="width: {span.days.length * dayWidth}px"
          >
            <div
